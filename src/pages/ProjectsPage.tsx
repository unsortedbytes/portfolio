...
/* Spotlight card — radial glow follows cursor inside card */
...
const SpotlightCard: React.FC<SpotlightProps> = ({ children, href, className = '' }) => {
    ...
    const onMove = (e: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        el.style.background = `radial-gradient(320px circle at ${x}px ${y}px, rgba(255,94,26,0.09), transparent 65%), #1A1A1A`;
    };
            ...
            <div className="max-w-5xl mx-auto px-6 pt-32 pb-16 relative z-10">
                ...
                <ScrollReveal direction="fade">
                    <div className="mb-16 text-center">
                        ...
                        <p className="text-zinc-500 text-sm max-w-md mx-auto">
                            Click any card to open the live project.
                        </p>
                    </div>
                </ScrollReveal>
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                    {liveProjects.map((project, i) => (
                        <ScrollReveal key={project.id} delay={i * 100} direction={i % 2 === 0 ? 'left' : 'right'}>
                            <SpotlightCard
                                href={project.liveUrl}
                                className="hover-lift block bg-zinc-900 border border-zinc-800 hover:border-amber-400/40 rounded-2xl p-6 group transition-colors duration-300 h-full"
                            >
                                {/* Top row */}
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    <span className="text-5xl font-black text-zinc-800 group-hover:text-zinc-700 transition-colors duration-300 leading-none font-mono select-none">
                                        {project.num}
                                    </span>
                                </div>

                                {/* Title */}
                                <h2 className="text-xl font-bold text-white group-hover:text-amber-400 transition-colors duration-300 mb-2">
                                    {project.title}
                                </h2>
                                ...
                                <div className="flex items-center justify-between mt-auto">
                                    <div className="flex items-center gap-1.5 text-xs font-semibold text-zinc-950 bg-amber-400 group-hover:bg-amber-300 px-4 py-1.5 rounded-full transition-colors duration-200">
                ...
                <ScrollReveal delay={400} direction="fade">
                    <div className="pt-12 flex items-center justify-between border-t border-zinc-800 mt-4">
                        <p className="text-zinc-700 font-mono text-xs">{liveProjects.length} projects · all deployed</p>
                        <a
