...
const GitHubActivity: React.FC = () => {
  ...
  const getColor = (level: number) => {
    const colors = [
      ...
      'bg-amber-900/50',  // 1-3 contributions
      'bg-amber-700/60',  // 4-6 contributions
      'bg-amber-500/70',  // 7-10 contributions
              ...
              {Array.from({ length: 7 }).map((_, dayIndex) => {
                ...
                return (
                  <div
                    ...
                    className={`w-3 h-3 rounded-sm ${getColor(contribution?.level || 0)} 
                      hover:ring-2 hover:ring-amber-400 transition-all duration-200 cursor-pointer`}
