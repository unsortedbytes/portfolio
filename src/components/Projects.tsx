      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {projects.map((project, index) => (
            <div key={index} className="bg-zinc-800/50 backdrop-blur-sm border border-zinc-700 rounded-lg shadow-lg p-6 hover:shadow-2xl hover:border-amber-500/50 transition duration-300 flex flex-col">
              <div className="flex justify-between items-start mb-3">
                <h3 className="text-2xl font-bold text-white">{project.title}</h3>
                <span className="text-xs text-zinc-400 bg-zinc-700 px-2 py-1 rounded">{project.year}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.tags.map((tag, i) => (
                  <span key={i} className="bg-amber-500/20 text-amber-300 border border-amber-500/30 px-3 py-1 rounded-md text-xs font-medium">
                    {tag}
                  </span>
                ))}
              </div>
              <div className="flex gap-4 mt-auto">
                {project.github && (
                  <a
                    ...
                    className="flex items-center text-zinc-300 hover:text-amber-400 transition duration-300"
                  >
                ...
                {project.demo && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center text-zinc-300 hover:text-amber-600 transition duration-300"
                  >
